...
export default function StatsCards({ stats, onFilter, activeType, activeValue }: StatsCardsProps) {
  return (
        ...
        {cards.map((card) => {
          ...
          return (
            <button
              key={card.key}
              onClick={() => {
                // Clicking an already-active card (other than Total) clears the filter
